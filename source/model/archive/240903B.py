...
class Trainer:
    ...
    hyperParams = HyperParams(features=768, expandBy=256, activate=32, relevant=16)
    trainParams = TrainParams(batchSize=256, numEpochs=128, learnRate=1e-3)
    ...
    def __init__(self) -> None:
        ...
        wandb.init(project="private", entity="esae", name=name)
    ...
    def trainLoss(
        ...
        loss["KLD"] = torch.tensor(0.0, requires_grad=True)
        buf = torch.exp(
            torch.matmul(
                qry.unsqueeze(1),
                docs.transpose(1, 2),
        ...
        for i in range(qry.size(0)):
            for j in range(docs.size(1)):
                ...
                loss["KLD"] = loss["KLD"] + F.kl_div(ins, tar, reduction="batchmean")
        return loss
